...
v_max = np.zeros(len(Crr_array), dtype=np.float64)
terrain_slope = 0.0
...
for i, crr in enumerate(Crr_array):
    def f_net_obj(omega_val):
        res = F_net(np.array([omega_val]), np.array([terrain_slope]), rover, planet, crr)
...
plt.plot(Crr_array, v_max, 'r-o', linewidth=2)
plt.title('Maximum Attainable Rover Speed vs. Rolling Resistance')
